            ...
            return `
                <article class="product-card text-right group opacity-0 animate-fade-in-up" style="animation-delay: ${index * 50}ms">
                    <div class="relative overflow-hidden rounded-custom bg-white shadow-md hover:shadow-xl transition-all duration-300">
                        <!-- زر المفضلة -->
                        <div class="absolute top-3 right-3 z-20">
                            <button onclick="event.stopPropagation(); FavoritesManager.toggle('${p.id}'); updateBadge(); showToast('${isFav ? 'تم الإزالة من المفضلة' : 'تمت الإضافة للمفضلة'}');" data-favorite-btn="${p.id}" class="favorite-btn p-2.5 rounded-full shadow-lg transition-all duration-300 ${isFav ? 'favorite-active' : ''}" title="${isFav ? 'إزالة من المفضلة' : 'أضف للمفضلة'}">
                                ${isFav 
                                    ? '<svg class="heart-icon w-5 h-5 fill-current text-white" viewBox="0 0 24 24"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>'
